...
/**
 * Creates dataset B+ from dataset A.
 *
 * B+ contains only the rows of A for which NSmells > 0.
 *
 * Default paths, when launched from the project root:
 *   input : milestone-1/output/A.csv
 *   output: milestone-3/output/B_plus.csv
 *
 * Optional arguments:
 *   java CreateDatasetBPlus.java [input.csv] [output.csv]
 */
public final class CreateDatasetBPlus {
    ...
    private static final Path DEFAULT_OUTPUT = Paths.get("milestone-3", "output", "B_plus.csv");

    private CreateDatasetBPlus() {
        // Utility class.
    }
    ...
    private static Result createDataset(final Path input, final Path output) throws IOException {
        ...
        try (CsvReader reader = new CsvReader(input);
            ...
            List<String> row;
            long recordNumber = 1; // Header is record 1.
    ...
    private static Path[] resolvePaths(final String[] args) {
        if (args.length > 2) {
            throw new IllegalArgumentException(
                    "Uso: java CreateDatasetBPlus.java [input.csv] [output.csv]");
        }
    ...
    private static BigDecimal parseNSmells(final String rawValue, final long recordNumber) {
        ...
        if (value.isEmpty()) {
            throw new IllegalArgumentException(
                    "NSmells mancante nel record CSV " + recordNumber + ".");
        }
    ...
    /** Minimal RFC-4180-style CSV reader supporting quoted commas and embedded newlines. */
