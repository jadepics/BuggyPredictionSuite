...
/**
 * Creates dataset B from dataset B+.
 *
 * B contains the same rows and columns as B+, but every NSmells value is forced to 0.
 * The script validates that every input row really belongs to B+ (NSmells > 0).
 *
 * Default paths, when launched from the project root:
 *   input : milestone-3/output/B_plus.csv
 *   output: milestone-3/output/B.csv
 *
 * Optional arguments:
 *   java CreateDatasetB.java [input-B-plus.csv] [output-B.csv]
 */
public final class CreateDatasetB {

    private static final String TARGET_COLUMN = "NSmells";
    private static final Path DEFAULT_INPUT = Paths.get("milestone-3", "output", "B_plus.csv");
    private static final Path DEFAULT_OUTPUT = Paths.get("milestone-3", "output", "B.csv");

    private CreateDatasetB() {
        // Utility class.
    }
    ...
    public static void main(final String[] args) {
        try {
            ...
            long rowsWritten = createDataset(paths[0], paths[1]);

            System.out.printf(Locale.ROOT,
                    "Dataset B creato correttamente.%n"
                            + "Input B+: %s%n"
                            + "Output B: %s%n"
                            + "Righe copiate: %d%n"
                            + "NSmells forzato a 0 in tutte le righe.%n",
                    paths[0].toAbsolutePath().normalize(),
                    paths[1].toAbsolutePath().normalize(),
                    rowsWritten);
        } catch (Exception exception) {
            System.err.println("Errore durante la creazione del dataset B: " + exception.getMessage());
            System.exit(1);
        }
    }

    private static long createDataset(final Path input, final Path output) throws IOException {
            ...
            while ((row = reader.nextRecord()) != null) {
                ...
                if (nSmells.compareTo(BigDecimal.ZERO) <= 0) {
                    throw new IllegalArgumentException(
                            "Il record CSV " + recordNumber
                                    + " non appartiene a B+: NSmells deve essere > 0, trovato " + nSmells + ".");
                }

                List<String> manipulatedRow = new ArrayList<>(row);
                manipulatedRow.set(nSmellsIndex, "0");
                writer.writeRecord(manipulatedRow);
                rowsWritten++;
            }
        } catch (Exception exception) {
    ...
    private static Path[] resolvePaths(final String[] args) {
        if (args.length > 2) {
            throw new IllegalArgumentException(
                    "Uso: java CreateDatasetB.java [input-B-plus.csv] [output-B.csv]");
        }
    ...
    private static void validateInputAndOutput(final Path input, final Path output) throws IOException {
        if (!Files.isRegularFile(input)) {
            throw new IllegalArgumentException(
                    "Dataset B+ non trovato. Esegui prima CreateDatasetBPlus: " + input.toAbsolutePath());
        }
        Path normalizedInput = input.toAbsolutePath().normalize();
        Path normalizedOutput = output.toAbsolutePath().normalize();
        if (normalizedInput.equals(normalizedOutput)) {
            throw new IllegalArgumentException("Input B+ e output B devono essere file diversi.");
        }
    ...
    private static List<String> requireHeader(final CsvReader reader) throws IOException {
        ...
        if (header == null) {
            throw new IllegalArgumentException("Il CSV B+ è vuoto e non contiene un header.");
        }
        if (!header.isEmpty()) {
